...
--branch	选填，项目分支
--heap-size  选填，jvm选项：Xmx。默认堆大小为0.8 * 内存大小。
--work_space  选填，默认为当前目录。
```
...
该存储库包括几个配置文件，以帮助您设置和自定义部署。这些文件位于 `config` 目录中：

- `main_net_config.conf`：主网络的配置文件。
- `private_net_config.conf`：私有网络的配置文件。
- `test_net_config.conf`：测试网络的配置文件。

### 配置示例

以下是一个配置文件 (`main_net_config.conf`) 的示例：
...
如果在部署过程中遇到任何问题，以下是一些常见的故障排除提示：

1. **检查依赖项**：确保系统上安装了所有必需的依赖项，例如 `java`、`git` 和 `wget`。如果缺少任何依赖项，请安装它们并再次运行脚本。

2. **验证网络连接**：如果遇到网络相关问题，例如下载失败或连接超时，请检查网络连接并重试。您还可以在脚本中为网络相关命令添加重试，以处理临时网络问题。

3. **验证配置文件**：确保配置文件已正确设置并包含有效值。仔细检查配置文件中的路径、端口和其他设置。

4. **检查日志**：查看部署过程中生成的日志文件，查找任何错误消息或警告。日志文件可以提供有价值的信息，帮助您识别和解决问题。

5. **增加详细程度**：如果脚本未提供足够的错误信息，请考虑通过添加更详细的错误消息和日志来增加脚本的详细程度。

6. **查阅文档**：参考官方文档和用户指南，获取更多信息和故障排除步骤。`docs` 目录包含详细的文档文件，例如用户指南、开发者指南和 API 文档。

通过遵循这些故障排除提示，您应该能够解决部署过程中遇到的大多数常见问题。
